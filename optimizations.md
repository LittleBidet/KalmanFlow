...
so callers can consume each filtered inflow and finalized fixed-lag outflow as
they become available. However, it adds allocation and remapping
...
state in the existing array-based filter result, and writes inflow, outflow,
and provenance flags into preallocated NumPy columns. Pandas receives those
