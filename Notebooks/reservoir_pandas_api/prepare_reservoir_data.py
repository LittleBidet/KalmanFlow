"""Prepare Aquarius reservoir exports for :mod:`kalmone.pandas_api`.

The functions in this module keep source-specific parsing and alignment outside
the Kalmone package.  ``prepare_reservoir_data`` returns a frame with exactly
the ``storage`` and ``outflow`` columns accepted by ``run_inflow_model``.
"""
...
def sources_for_reservoir(project_root: Path, reservoir: str) -> ReservoirSources:
    ...
    missing = [path for path in _source_paths(sources).values() if not path.is_file()]
    if missing:
    ...
    parsed = pd.DataFrame(
        {
            "timestamp": pd.to_datetime(
                raw["ISO 8601 UTC"], utc=True, errors="coerce"
            ),
...
def prepare_reservoir_data(
    ...
    *,
    ...
    """Clean and align a closed UTC time window for ``run_inflow_model``.

    Storage timestamps define the output clock. Other series are matched to the
    most recent value at or before each storage timestamp, bounded by
    ``asof_tolerance``. ``start`` and ``end`` must be timezone-aware, and both
    ...
    spillway = _backward_asof(clock, cleaned["spillway"], "spillway_flow", tolerance)
    ...
    if sources.combine_spillway:
        diagnostics["outflow"] = (
            diagnostics["outlet_discharge"] + diagnostics["spillway_flow"]
        )
        outflow_definition = "outlet discharge + spillway flow"
    else:
        diagnostics["outflow"] = diagnostics["outlet_discharge"]
        outflow_definition = (
            "downstream discharge only; spillway retained as diagnostic"
        )
...
def _source_paths(sources: ReservoirSources) -> dict[str, Path]:
    return {
        "storage": Path(sources.storage),
        "outlet": Path(sources.outlet),
        "spillway": Path(sources.spillway),
